...
export class InvoicesComponent {
  ...
  addDetail(){
    ...
    const depot = this.depots.find(p => p.id == this.createDetail.productId);
    if (depot) {
  ...
  addUpdateDetail(){
    ...
    const depot = this.depots.find(p => p.id == this.updateDetail.productId);
    if (depot) {
      this.createDetail.depot = depot;
    }
  ...
  deleteById(model: InvoiceModel){
    this.swal.callSwal("Siparişi Sil", `${model.customer.name} - ${model.invoiceNumber} numaralı faturayı silmek istiyor musunuz?`, () => {
